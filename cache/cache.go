// Package cache implements a dns cache that keeps records in memory for their TTL duration
// this is based on the cache implementation from https://github.com/AdguardTeam/AdGuardDNS
package cache
...
// Cache holds the cache objects in memory
type Cache struct {
...
// Get returns the dns response for the provided request if found
func (c *Cache) Get(request *dns.Msg) (*dns.Msg, bool) {
	...
	if err != nil {
		// real error
		log.Printf("can't get response for %s from cache: %s", request.Question[0].Name, err)
		return nil, false
	}

	cachedValue, ok := rawValue.(item)
	if !ok {
		log.Printf("entry with invalid type in cache for %s", request.Question[0].Name)
		return nil, false
	}
...
// Set saves a new dns message (question/answer) into the cache
func (c *Cache) Set(m *dns.Msg) {
	...
	if err != nil {
		log.Printf("Couldn't set cache item")
	}
}
...
func isCacheable(m *dns.Msg) bool {
	...
	if m.Rcode != dns.RcodeSuccess && m.Rcode != dns.RcodeNameError {
		log.Printf("%s: refusing to cache message with response type %s", qName, dns.RcodeToString[m.Rcode])
		return false
	}
	...
	if m.Rcode == dns.RcodeSuccess && (qType == dns.TypeA || qType == dns.TypeAAAA) {
		...
		if len(m.Answer) == 0 {
			log.Printf("%s: refusing to cache a NOERROR response with no answers", qName)
			return false
		}
		...
		if !found {
			log.Printf("%s: refusing to cache a response with no A and AAAA answers", qName)
			return false
		}
	}
